...
def predict(config):
    ...
    for tag, generator in predict_gen.items():
        ...
        if tag in output_conf:
            ...
            elif output_conf[tag]['save_format'] == 'SEMEVAL':
                with open(output_conf[tag]['save_path'], 'w') as f:
                    temp = []
                    
                    for qid, dinfo in res_scores.items():
                            dinfo = sorted(dinfo.items(), key=lambda d:d[1][0], reverse=True)
                            for rank,(qaid, (score, rel)) in enumerate(dinfo):
                                temp.append([qid[1:], qaid[2:], rank, score, 'true' if rel else 'false'])
                    
                    temp.sort(key=lambda d:int(d[1]))
                    temp.sort(key=lambda d:int(d[0]), reverse=True)

                    for qid, qaid, rank, score, rel in temp:
                        f.write('%s\t%s\t%d\t%f\t%s\n'%(qid, qaid, rank, score, rel))
